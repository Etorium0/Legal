...
import (
	...
	wtranscriber "github.com/dvonthenen/open-virtual-assistant/pkg/transcriber/whisper"
)
...
func New(assistantImpl *ainterfaces.AssistantImpl, opts *AssistantOptions) (*Assistant, error) {
	...
	case ainterfaces.WHISPER_TRANSCRIBER:
		transcribe, errTranscribe := wtranscriber.New(ctx, assistant.transcriberOptions)
		if errTranscribe != nil {
			klog.V(1).Infof("wtranscriber.New failed. Err: %v\n", errTranscribe)
			return nil, errTranscribe
		}
		transcriber = transcribe
